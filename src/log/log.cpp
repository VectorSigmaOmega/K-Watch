...
void emit(Level l, const char *component, const std::string &msg) {
    ...
    (void)std::fprintf(stderr, "%s.%03ldZ %s %s %s\n", ts_buf, (long)ms.count(), level_to_string(l),
                       component, msg.c_str());
}

static int libbpf_print_fn(enum libbpf_print_level level, const char *format, va_list args) {
    if (level == LIBBPF_DEBUG && current_level < Level::Debug)
        return 0;
    if (level == LIBBPF_INFO && current_level < Level::Info)
        return 0;
...
void init_libbpf_logging() { libbpf_set_print(libbpf_print_fn); }
